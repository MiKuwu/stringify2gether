      ...
      <div className="flex gap-4 mb-6">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-white pl-10 p-3 rounded focus:outline-none focus:border-teal-500 transition"
          />
        </div>
      </div>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-4 mt-8">
          <button 
            ...
            className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-white rounded disabled:opacity-50 hover:bg-slate-200 dark:bg-slate-700 transition"
          >
            Trang trước
          </button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl shadow-2xl max-w-sm w-full overflow-hidden animate-in zoom-in-95 duration-200 relative">
            <button 
              ...
              className="absolute top-4 right-4 text-slate-600 dark:text-slate-400 hover:text-white transition"
            >
              <X size={20} />
            </button>
            
            <div className="p-6">
              <h3 className="text-xl font-bold text-white mb-6 border-b border-slate-300 dark:border-slate-700 pb-4 pr-6">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <div className="font-bold text-white">{breakdownUser.username || "Ẩn danh"}</div>
              ...
              <div className="space-y-3">
                {Object.entries(getBreakdown(breakdownUser.logs)).map(([action, count]) => (
                  <div key={action} className="flex justify-between items-center bg-slate-100 dark:bg-slate-800 p-3 rounded border border-slate-300 dark:border-slate-700/50">
                    ...
                    <span className="font-black text-white text-lg">x{count}</span>
                  </div>
                ))}
            ...
            <div className="p-4 bg-slate-100 dark:bg-slate-800/50 text-center">
              <button 
                onClick={() => setBreakdownUser(null)}
                className="w-full py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-600 text-white rounded font-bold transition"
              >
                Đóng
              </button>
            </div>
